define("dist/js/sign", [ "libs/jquery","libs/jquery-plugins/jqBootstrapValidation","libs/jquery-plugins/jquery.backstretch.min"], function(require) {
    ...
    $.backstretch([ "img/bg1.png", "img/bg2.png" ], {
        duration: 3e3,
        fade: 750
    });
    $(".logo").tooltip();
    ...
    $("[name=passwordsignup_confirm]").jqBootstrapValidation({
        validationHandlers: [ "blur" ]
    });
    ...
    $("#login form").submit(function(e) {
        ...
        $.post("/login", {
            ...
            if (res == "err_id") {
                $("#username").parent(".control-group").addClass("error").find(".help-block").html('<ul role="alert"><li>用户名不存在</li></ul>');
            } else if (res == "err_pw") {
                $("#password").parent(".control-group").addClass("error").find(".help-block").html('<ul role="alert"><li>密码错误</li></ul>');
            } else if (res) {
                location.reload();
            }
        });
    ...
    $("#usernamesignup").on("keyup paste", function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (!data) {
                    $that.parent(".control-group").addClass("warning").removeClass("success").find(".help-block").html('<ul role="alert"><li>用户名已存在</li></ul>');
                } else {
                    $that.parent(".control-group").removeClass("warning").find(".help-block").html("");
                }
            },
            complete: function() {
